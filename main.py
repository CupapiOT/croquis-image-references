...
class App(ctk.CTk):
    def __init__(
        ...
        self.mid_window_width = int(self.winfo_screenwidth()
                                    / 2 - geometry[0] / 2)
        self.mid_window_height = int(self.winfo_screenheight()
                                     / 2 - geometry[1] / 2 - 40)
        ...
        self.image_frame.pack(
            ...
            expand=True,
            fill='both',
            before=self.everything_else_frame
            )
        ...
        if (self.save_load_system.values['directories']
                and self.save_load_system.values['load_saved_directories']):
            self.timer = Timer(
                interval=float(self.save_load_system.values
                               ['wait_directory_load']),
    ...
    def create_normal_layout(self):
        ...
        # timer Frame
        self.timer_frame.timers_frame.pack(
            expand=False,
            fill='both',
            side='top'
            )
        ...
        self.timer_frame.start_button.pack(
            padx=5,
            side='left',
            before=self.timer_frame.pause_button
            )
    ...
    def create_wide_layout(self):
        ...
        self.timer_frame.buttons_frame.pack(
            expand=False,
            fill='y',
            side='left'
            )
        ...
        self.timer_frame.second_label.pack(
            side='left',
            padx=3
            )
    ...
    def open_folder(self,
                    reset_queue=False,
                    predetermined_folder=None) -> None:
        ...
        try:
            ...
            folder = predetermined_folder \
                if predetermined_folder is not None \
                else (filedialog.askdirectory(title='Select A Folder',
            ...
            if folder:
                ...
                self.image_frame.image_amount = len(
                    self.image_frame.image_amount_list
                    )
                self.image_frame.frame_index = \
                    self.image_frame.image_amount_list[0]

                # values = root.save_load_system.values
                if predetermined_folder is None:
                    self.save_load_system.values['directories'].append(
                        str(folder))
                    self.save_load_system.save_value(
                        input_value=str(self.save_load_system.values),
                        file_name=self.save_load_system.file_name)
                ...
                self.settings_menu.image_limit_state(
                    normal_or_disabled='normal'
                    )
                ...
                self.settings_menu.switch_folder_button.configure(
                    state='normal',
                    fg_color=self.button_theme_color['normal']
                    )
...
class ImageFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent):
        ...
        self.parent = parent
        super().__init__(
            master=parent, fg_color='transparent',
        ...
        self.directory_list = []
        self.image_amount_list = list(range(0, len(self.directory_list))) \
            if list(range(0, len(self.directory_list))) \
            else []
        self.image_amount = len(self.image_amount_list)
        self.randomize_list_bool = ctk.BooleanVar(
            value=self.values['randomize'])
        self.image_limit_display = self.values['image_limit'] \
            if self.values['image_limit'] != 'Unlimited' \
            else 'Unlimited'
        self.image_limit_entry_var = ctk.StringVar(
            value=self.image_limit_display
        ...
        self.image_original = Image.open(
            'other_essentials/empty_placeholder.png')
        self.image_ratio = (self.image_original.size[0] /
        ...
        self.image_displayer_colors: dict = {'Light': '#EBEBEB',
                                             'Dark': '#242424'
                                             }
        ...
        self.image_displayer.create_image(0, 0, anchor='nw',
                                          image=self.image_tk)
    ...
    def show_full_image(self,
                        width: int,
                        height: int,
                        turn_off_settings_menu=False) -> None:
    ...
    def save_image_limit(self) -> None:
        try:
            image_limit = int(
                self.parent.settings_menu.image_limit_entry.get()) \
                if (self.parent.settings_menu.image_limit_entry.get()
                    != 'Unlimited') \
                else 0
            if self.image_amount_list:
                self.image_limit_display = 'Unlimited' if (
                        0 >= image_limit or image_limit >=
                        self.image_amount) else str(image_limit)
            else:
                self.image_limit_display = 'Unlimited' if 0 >= image_limit \
            ...
            self.image_limit_display = min(int(self.image_limit_display),
                                           15_000_000)
        ...
        self.parent.settings_menu.image_limit_entry.insert(
            index=0,
            string=self.image_limit_display
            )
        ...
        # current frame index
        if self.image_amount_list:
            if (self.image_limit_display != 'Unlimited' and self.frame_index >=
                    int(self.image_limit_display)):
...
class TimerFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent: any, window: any):
        ...
        self.temp = int
        self.save_temp = int(self.values['timer_temp']) \
        ...
        self.entry_color_dict = {'Light': {'normal': '#000000',
                                           'readonly': '#6C6C6C',
                                           'disabled': '#D8D8D8'
                                           },
                                 'Dark': {'normal': '#FFFFFF',
        ...
        self.label_color_dict = {'Dark': {'normal': '#FFFFFF',
                                          'readonly': '#888888',
                                          'disabled': '#ABABAB'
                                          },
                                 'Light': {'normal': '#000000',
        ...
        self.reset_bind_dictionary = {
            'normal': 'root.timer_frame.reset_time('
                      'turn_off_settings_menu=True)',
        ...
        self.start_time_image = ctk.CTkImage(
            light_image=Image.open(
                'app_widget_images/time_buttons/start_button.png'),
            dark_image=Image.open(
                'app_widget_images/time_buttons/start_button.png')
            )
        self.pause_start_time_image = ctk.CTkImage(
            light_image=Image.open(
                'app_widget_images/time_buttons/pause_button.png'),
            dark_image=Image.open(
                'app_widget_images/time_buttons/pause_button.png')
            )
        ...
        ToolTip(widget=self.start_button,
                msg='Start Timer (Ctrl+S)',
                delay=0.3
                )
        ...
        ToolTip(widget=self.pause_button,
                msg='Pause/Play Timer (Space)',
                delay=0.3)
        ...
        ToolTip(widget=self.reset_button,
                msg='Reset Timer (Ctrl+R)',
                delay=0.3)
    ...
    def time_button_state(self,
                          start_state: str,
                          pause_state: str,
                          reset_state: str) -> None:
        assert ((start_state, pause_state, reset_state) == 'normal' or
                'disabled')
        ...
        self.start_button.configure(
            fg_color=self.root.button_theme_color[start_state]
            )
        self.pause_button.configure(
            fg_color=self.root.button_theme_color[pause_state]
            )
        self.reset_button.configure(
            fg_color=self.root.button_theme_color[reset_state]
            )

        # setting the button shortcuts
        exec(f"self.root.bind('<Control-KeyPress-s>', "
             f"lambda _: {self.start_bind_dictionary[start_state]})")
        exec(f"self.root.bind('<space>', "
             f"lambda _: {self.pause_bind_dictionary[pause_state]})")
        exec(f"self.root.bind('<Control-KeyPress-r>', "
    ...
    def start_time(self,
        ...
        try:
            if start_button_not:
                if (int(self.hour.get()) * 3600
                        + int(self.minute.get()) * 60
                        + int(self.second.get()) <= 0
                        and not pause_button_not):
                    self.save_temp = 30
                else:
                    self.save_temp = (int(self.hour.get()) * 3600
                                      + int(self.minute.get()) * 60
                                      + int(self.second.get()))
            ...
            self.temp = (int(self.hour.get()) * 3600
                         + int(self.minute.get()) * 60
                         + int(self.second.get()))
                ...
                if not self.paused:
                    ...
                    if self.temp == 0:
                        ...
                        if not self.time_start:
                            self.timer = Timer(
                                1,
                                self.reduce_temp_by_one_second
                                )
                            self.timer.start()
    ...
    def heads_up_countdown(self):
        ...
        self.time_entry_state('readonly')
        self.time_button_state(start_state='disabled',
                               pause_state='disabled',
                               reset_state='disabled')
        self.root.settings_menu.image_limit_state(
            normal_or_disabled='disabled')

        # counts down 3 seconds
        while self.temp >= 0:
            if self.temp > 0:
                root.image_frame.image_original = Image.open(
                    self.countdown_image_dict[ctk.get_appearance_mode()][
                        self.temp]
                    )
...
class ButtonFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent: any, image_button_parent: any, window: any):
        ...
        self.next_image_png = ctk.CTkImage(Image.open(
            'app_widget_images/change_image_buttons/next_image_button.png'),
            Image.open(
                'app_widget_images/change_image_buttons/'
                'next_image_button.png'))
        self.first_image_png = ctk.CTkImage(Image.open(
            'app_widget_images/change_image_buttons/first_image_button.png'),
            Image.open(
                'app_widget_images/change_image_buttons/'
                'first_image_button.png')
            )
        self.last_image_png = ctk.CTkImage(Image.open(
            'app_widget_images/change_image_buttons/'
            'last_image_button.png'),
            Image.open(
                'app_widget_images/change_image_buttons/'
                'last_image_button.png')
            )
        ...
        ToolTip(widget=self.previous_image_button,
                msg='Previous Image',
                delay=0.3
                )
        ...
        self.index_label = ctk.CTkLabel(
            parent,
            text='Select an image folder from the folder menu'
            )
    ...
    def update_index_label(self):
        ...
        try:
            ...
            # in "n image out of x"
            out_of_n = root.image_frame.image_amount \
            ...
            if root.image_frame.frame_index == \
                    root.image_frame.reachable_images[
                        -1] and not root.timer_frame.paused \
            ...
            # else if it's paused (+ if it's also on loop)
                ...
                end_modifier = ' (Paused)' if not self.loop_or_not.get() \
                    else ' (Paused, On Loop)'
            ...
            if root.image_frame.image_limit_display != 'Unlimited':
                end_modifier = \
                    f' ({root.image_frame.image_amount}) {end_modifier}'
            ...
            print('IndexLabelUpdateError: No folder has been selected yet.\n'
                  '\t\t\t\t\t   Nothing should go wrong, though.')

    def image_button_state(self, normal_or_disabled: str) -> None:
        # setting the time entry states
        for button in ['first_image_button',
                       'previous_image_button',
                       'next_image_button',
                       'last_image_button']:
            exec(f"self.{button}.configure(state=normal_or_disabled,"
        ...
        if normal_or_disabled == 'normal':
            ...
            self.root.bind(
                '<Down>',
                lambda _: root.button_frame.change_image(first_or_last=True)
                )
            self.root.bind(
                '<Right>',
            ...
            self.root.bind(
                '<Left>',
                lambda _: root.button_frame.change_image(
                    next_or_previous=False
                    )
                )
            self.root.bind(
                '<Up>',
                lambda _: root.button_frame.change_image(first_or_last=False)
                )
            ...
            self.root.bind(
                '<Down>',
                lambda _: print('First Image shortcut currently disabled.')
                )
            self.root.bind(
                '<Right>',
            ...
            self.root.bind(
                '<Left>',
                lambda _: print('Next Image shortcut currently disabled.')
                )
            self.root.bind(
                '<Up>',
                lambda _: print('Last Image shortcut currently disabled.')
                )

    def update_image_original(self) -> None:
        self.root.image_frame.image_original = Image.open(
            self.root.image_frame.directory_list
            [self.root.image_frame.frame_index]
            )
        ...
        try:
            if next_or_previous or first_or_last:
                ...
                # if loop is on or if the button = first
                    ...
                    if (root.image_frame.randomize_list_bool.get()
                            and first_or_last is None):
                ...
                # if loop is on or if the button = last
                    ...
                    root.image_frame.frame_index = \
                        root.image_frame.reachable_images[-1]
            ...
            print('ImageChange Error: No folder has been selected yet to be '
...
class SizeNotifier:
    def __init__(self, parent: any, size_dictionary: dict):
        self.root = parent
        self.size_dict = {pixels: layout for pixels, layout in
                          sorted(size_dictionary.items())}
...
class SettingsMenu(ctk.CTkButton):
    def __init__(self,
        ...
        self.load_saved_temp_bool = ctk.BooleanVar(
            value=self.values['load_timer_temp']
            )
        ...
        ToolTip(self.image_limit_save_button,
                msg='Save Image Limit (Enter)',
                delay=0.3)
        ...
        # layout
        # # buttons menu layout
        for button in [self.add_new_folder_button,
                       self.switch_theme,
        ...
        for checkbox in [self.countdown_checkbox,
        ...
        self.load_timer_temp_checkbox.pack(
            expand='True',
            fill='x',
            padx=5,
            pady=2
    ...
    def image_limit_state(self, normal_or_disabled: str):
        self.image_limit_entry.configure(
            ...
            if normal_or_disabled == 'normal' else ('#888888', '#666666')
            )
        if normal_or_disabled == 'normal':
            self.image_limit_entry.bind(
                '<Return>',
    ...
    def load_saved_directories_not(self):
        ...
        self.values[
            'load_saved_directories'] = self.load_saved_directories_bool.get()
        self.parent.save_load_system.save_value(
            input_value=str(self.values),
            file_name=self.parent.save_load_system.file_name
            )
...
def switch_theme(switch_theme_not=True):
    ...
    for entry in (root.timer_frame.hour_entry, root.timer_frame.minute_entry,
    ...
    # switch
    for label in (root.timer_frame.hour_label, root.timer_frame.minute_label,
                  root.timer_frame.second_label):
        label.configure(text_color=root.timer_frame.label_color_dict[
            ctk.get_appearance_mode()][root.timer_frame.current_entry_color[1]]
            )
